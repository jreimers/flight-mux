...
def clean_cache() -> None:
    for file in tqdm(os.listdir('./cached/')):
        if file[-4:] == 'json':
            ...
            data = df.loc[
                df[['Leave Date', 'Return Date', 'Access Date', 'Depart Time (Leg 1)', 'Arrival Time (Leg 1)']].astype(
                    str).drop_duplicates().index].to_dict()
...
def check_cached(origin: str, dest: str, date_leave, date_return):
    ...
    if isinstance(date_leave, list) and isinstance(date_return, list):
        return np.all([cache_condition(df=df, date_leave=date_leave[i], date_return=date_return[i]) for i in
                       range(len(date_leave))])
...
def scrape_data(origin, dest, date_leave, date_return, cache=False) -> dict:
    ...
    if isinstance(date_leave, list) and isinstance(date_return, list):
        ...
        url = [make_url(origin=origin, dest=dest, date_leave=date_leave[i], date_return=date_return[i]) for i in
               range(len(date_leave))]
